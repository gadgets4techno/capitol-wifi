...
def send(str = None,retain=False):	#publishes provided message to pre-defined topic
 client.publish(server_sub_topic, str,retain=retain)
...
def on_connect(client, userdata, flags, rc):	#do smth when connecting to broker
 global connected
 if (rc == 0):
  print("Connection OK, Return code = " + str(rc))
